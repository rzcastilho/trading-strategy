defmodule TradingStrategy.BacktestingIntegrationTest do
  ...
  describe "trade data consistency" do
    ...
    test "position realized_pnl equals sum of trade PnLs" do
      ...
      trades = Repo.all(from t in Trade, where: t.position_id == ^position.id, order_by: t.timestamp)
      ...
      assert exit_trade.duration_seconds == 3600  # 1 hour in seconds
    ...
    test "multiple positions with multiple trades maintain consistency" do
      ...
      position1 = create_position_with_trades(session.id, %{
      ...
      position2 = create_position_with_trades(session.id, %{
      ...
      position3 = create_position_with_trades(session.id, %{
      ...
      expected_total = Decimal.new("300.00")  # 200 - 100 + 200
